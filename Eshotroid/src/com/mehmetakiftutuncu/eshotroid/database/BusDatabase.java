...
/** A database class using singleton design pattern for all the bus information
 * 
...
public class BusDatabase implements IDatabaseOperations<Bus>
{
	...
	/** Key for the flag if weekday times of a bus exists */
	public static final String KEY_BUSSES_TIMESH_EXISTS = "busTimesHExists";
	/** Key for the flag if saturday times of a bus exists */
	public static final String KEY_BUSSES_TIMESC_EXISTS = "busTimesCExists";
	/** Key for the flag if sunday times of a bus exists */
	...
	private static final String CREATE_SQL = "CREATE TABLE " + TABLE_NAME_BUSSES
			+ " (" + KEY_BUSSES_NUMBER + " INTEGER PRIMARY KEY NOT NULL, "
					...
					+ KEY_BUSSES_TIMESH_EXISTS + " INTEGER NOT NULL,"
					+ KEY_BUSSES_TIMESC_EXISTS + " INTEGER NOT NULL,"
					+ KEY_BUSSES_TIMESP_EXISTS + " INTEGER NOT NULL" + ");";
	...
	/**	{@link BusDatabaseHelper} instance for creating/opening the database */
	 ...
	 * 
	 * @author mehmetakiftutuncu */
	private static class BusDatabaseHelper extends SQLiteOpenHelper
	{
		...
		{
			if(BuildConfig.DEBUG) Log.d(LOG_TAG, "Running create bus database SQL...");
		...
		{
			if(BuildConfig.DEBUG) Log.d(LOG_TAG, "Upgrading bus database...");
	...
	public static synchronized BusDatabase getDatabase(Context context) throws SQLException
	{
		if(myInstance == null)
		{
			if(BuildConfig.DEBUG) Log.d(LOG_TAG, "Creating a new BusDatabase instance...");
			myInstance = new BusDatabase();
		}
		
		if(myHelper == null)
		{
			if(BuildConfig.DEBUG) Log.d(LOG_TAG, "Creating a new BusDatabaseHelper instance...");
			myHelper = new BusDatabaseHelper(context);
		}
		
		if(!isOpened)
		{
			if(BuildConfig.DEBUG) Log.d(LOG_TAG, "Opening bus database...");
			isOpened = true;
	...
	public synchronized void closeDatabase()
	{
		if(isOpened)
		{
			if(BuildConfig.DEBUG) Log.d(LOG_TAG, "Closing bus database...");
			myHelper.close();
		}
	...
	@Override
	public synchronized boolean addOrUpdate(Bus entry)
	{
		/* Result flag */
		boolean result = true;
		...
		values.put(KEY_BUSSES_TIMESH_EXISTS, (entry.timesHExists() ? 1 : 0));
		values.put(KEY_BUSSES_TIMESC_EXISTS, (entry.timesCExists() ? 1 : 0));
		values.put(KEY_BUSSES_TIMESP_EXISTS, (entry.timesPExists() ? 1 : 0));
		
		try
		{
			/* Get the entry from the database */
			Bus bus = get(entry.getNumber());
			
			/* If the entry is already in the database */
			if(bus != null)
			{
				/* Updating */
				myDatabase.update(TABLE_NAME_BUSSES, values, KEY_BUSSES_NUMBER + "=" + entry.getNumber(), null);
			}
			else
			{
				/* Adding */
				myDatabase.insert(TABLE_NAME_BUSSES, null, values);
			}
		}
		catch(Exception e)
		{
	...
	public synchronized ArrayList<Bus> get()
	{
		...
		Cursor cursor = myDatabase.query(TABLE_NAME_BUSSES, columns, null, null, null, null, null);
		...
		for(cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext())
		{
			...
			list.add(new Bus(number, source, destination, null, isFavorited, null, null, null, true, true, true));
		}
	...
	public synchronized Bus get(int number)
	{
		...
		String[] columns = new String[]
		{
			...
			KEY_BUSSES_TIMESC,
			KEY_BUSSES_TIMESP,
			KEY_BUSSES_TIMESH_EXISTS,
			KEY_BUSSES_TIMESC_EXISTS,
			KEY_BUSSES_TIMESP_EXISTS
		};
		
		/* Cursor to query the database */
		Cursor cursor = myDatabase.query(TABLE_NAME_BUSSES, columns, KEY_BUSSES_NUMBER + "=" + number, null, null, null, null);
		...
		if(cursor != null)
		{
			...
			if(cursor.getCount() > 0)
			{
				...
				boolean timesHExists;
				boolean timesCExists;
				boolean timesPExists;
				...
				timesHExists = (cursor.getInt(cursor.getColumnIndex(KEY_BUSSES_TIMESH_EXISTS)) == 1);
				timesCExists = (cursor.getInt(cursor.getColumnIndex(KEY_BUSSES_TIMESC_EXISTS)) == 1);
				timesPExists = (cursor.getInt(cursor.getColumnIndex(KEY_BUSSES_TIMESP_EXISTS)) == 1);
				
				/* Generate the resulting item */
				item = new Bus(number, source, destination, route, isFavorited, timesH, timesC, timesP, timesHExists, timesCExists, timesPExists);
			}
	...
	@Override
	public synchronized boolean delete(int number)
	{
		/* Result flag */
		boolean result = true;
		
		/* Delete */
		int affectedRows = myDatabase.delete(TABLE_NAME_BUSSES, KEY_BUSSES_NUMBER + "=" + number, null);
		if(affectedRows == 0)
		{
			result = false;
		}
